import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
import itertools

class Swish(nn.Module):
    def forward(self, x):
        return x * torch.sigmoid(x)

class FCLayer(nn.Module):
    def __init__(self, input_dim, output_dim, ensemble_size, activation):
        super().__init__()
        self._input_dim = input_dim
        self._output_dim = output_dim
        self.weight = nn.Parameter(torch.Tensor(ensemble_size, input_dim, output_dim))
        self.bias = nn.Parameter(torch.Tensor(ensemble_size, output_dim))
        self._activation = activation
    def forward(self, x):
        return self._activation(torch.add(torch.bmm(x, self.weight), self.bias[:, None, :]))

def truncated_normal_init(t, mean=0.0, std=0.01):
    with torch.no_grad():
        torch.nn.init.normal_(t, mean=mean, std=std)
        while True:
            cond = (t < mean - 2 * std) | (t > mean + 2 * std)
            if not torch.sum(cond):
                break
            new_t = torch.empty_like(t)
            torch.nn.init.normal_(new_t, mean=mean, std=std)
            t[:] = torch.where(cond, new_t, t)

def init_weights(m):
    if isinstance(m, nn.Linear) or isinstance(m, FCLayer):
        truncated_normal_init(m.weight, std=1 / (2 * np.sqrt(m._input_dim)))
        if hasattr(m, "bias") and m.bias is not None:
            m.bias.data.fill_(0.0)

class EnsembleModel(nn.Module):
    def __init__(self, state_dim, action_dim, model_alpha, ensemble_size=5, learning_rate=1e-3):
        super().__init__()
        self._output_dim = (state_dim + 1) * 2
        self._model_alpha = model_alpha
        self._max_logvar = nn.Parameter(torch.ones((1, self._output_dim // 2)).float() / 2, requires_grad=False)
        self._min_logvar = nn.Parameter(-torch.ones((1, self._output_dim // 2)).float() * 10, requires_grad=False)
        self.layer1 = FCLayer(state_dim + action_dim, 200, ensemble_size, Swish())
        self.layer2 = FCLayer(200, 200, ensemble_size, Swish())
        self.layer3 = FCLayer(200, 200, ensemble_size, Swish())
        self.layer4 = FCLayer(200, 200, ensemble_size, Swish())
        self.layer5 = FCLayer(200, self._output_dim, ensemble_size, nn.Identity())
        self.apply(init_weights)
        self.optimizer = torch.optim.Adam(self.parameters(), lr=learning_rate)
    def forward(self, x, return_log_var=False):
        ret = self.layer5(self.layer4(self.layer3(self.layer2(self.layer1(x)))))
        mean = ret[:, :, : self._output_dim // 2]
        logvar = self._max_logvar - F.softplus(self._max_logvar - ret[:, :, self._output_dim // 2 :])
        logvar = self._min_logvar + F.softplus(logvar - self._min_logvar)
        if return_log_var:
            return mean, logvar
        return mean, torch.exp(logvar)
    def loss(self, mean, logvar, labels, use_var_loss=True):
        inverse_var = torch.exp(-logvar)
        if use_var_loss:
            mse_loss = torch.mean(torch.mean(torch.pow(mean - labels, 2) * inverse_var, dim=-1), dim=-1)
            var_loss = torch.mean(torch.mean(logvar, dim=-1), dim=-1)
            total_loss = torch.sum(mse_loss) + torch.sum(var_loss)
        else:
            mse_loss = torch.mean(torch.pow(mean - labels, 2), dim=(1, 2))
            total_loss = torch.sum(mse_loss)
        return total_loss, mse_loss
    def train_step(self, loss):
        self.optimizer.zero_grad()
        loss = loss + self._model_alpha * torch.sum(self._max_logvar) - self._model_alpha * torch.sum(self._min_logvar)
        loss.backward()
        self.optimizer.step()

class EnsembleDynamicsModel:
    def __init__(self, state_dim, action_dim, model_alpha=0.01, num_network=5, device=torch.device("cpu")):
        self._num_network = num_network
        self._state_dim = state_dim
        self._action_dim = action_dim
        self.device = device
        self.model = EnsembleModel(state_dim, action_dim, model_alpha, ensemble_size=num_network).to(device)
        self._epoch_since_last_update = 0
    def train(self, inputs, labels, batch_size=64, holdout_ratio=0.1, max_iter=20):
        permutation = np.random.permutation(inputs.shape[0])
        inputs = inputs[permutation]
        labels = labels[permutation]
        num_holdout = max(1, int(inputs.shape[0] * holdout_ratio))
        train_inputs = inputs[num_holdout:]
        train_labels = labels[num_holdout:]
        holdout_inputs = inputs[:num_holdout]
        holdout_labels = labels[:num_holdout]
        holdout_inputs = torch.from_numpy(holdout_inputs).float().to(self.device)
        holdout_labels = torch.from_numpy(holdout_labels).float().to(self.device)
        holdout_inputs = holdout_inputs[None, :, :].repeat([self._num_network, 1, 1])
        holdout_labels = holdout_labels[None, :, :].repeat([self._num_network, 1, 1])
        self._snapshots = {i: (None, 1e10) for i in range(self._num_network)}
        for epoch in itertools.count():
            train_index = np.vstack([np.random.permutation(train_inputs.shape[0]) for _ in range(self._num_network)])
            for batch_start_pos in range(0, train_inputs.shape[0], batch_size):
                batch_index = train_index[:, batch_start_pos : batch_start_pos + batch_size]
                train_input = torch.from_numpy(train_inputs[batch_index]).float().to(self.device)
                train_label = torch.from_numpy(train_labels[batch_index]).float().to(self.device)
                mean, logvar = self.model(train_input, return_log_var=True)
                loss, _ = self.model.loss(mean, logvar, train_label)
                self.model.train_step(loss)
            with torch.no_grad():
                mean, logvar = self.model(holdout_inputs, return_log_var=True)
                _, holdout_losses = self.model.loss(mean, logvar, holdout_labels, use_var_loss=False)
                holdout_losses = holdout_losses.detach().cpu().numpy()
            break_condition = self._save_best(epoch, holdout_losses)
            if break_condition or epoch > max_iter:
                break
    def _save_best(self, epoch, losses, threshold=0.1):
        updated = False
        for i in range(len(losses)):
            current = losses[i]
            _, best = self._snapshots[i]
            improvement = (best - current) / max(best, 1e-10)
            if improvement > threshold:
                self._snapshots[i] = (epoch, current)
                updated = True
        if updated:
            self._epoch_since_last_update = 0
        else:
            self._epoch_since_last_update += 1
        return self._epoch_since_last_update > 5
    def predict(self, inputs):
        inputs = np.tile(inputs, (self._num_network, 1, 1))
        inputs = torch.tensor(inputs, dtype=torch.float32, device=self.device)
        mean, var = self.model(inputs, return_log_var=False)
        return mean.detach().cpu().numpy(), var.detach().cpu().numpy()

class FakeEnv:
    def __init__(self, model):
        self.model = model
    def step(self, obs, act):
        obs = np.asarray(obs, dtype=np.float32).reshape(1, -1)
        act = np.asarray(act, dtype=np.float32).reshape(1, -1)
        inputs = np.concatenate((obs, act), axis=-1)
        ensemble_model_means, ensemble_model_vars = self.model.predict(inputs)
        ensemble_model_means[:, :, 1:] += obs
        ensemble_model_stds = np.sqrt(np.maximum(ensemble_model_vars, 1e-8))
        ensemble_samples = ensemble_model_means + np.random.normal(size=ensemble_model_means.shape) * ensemble_model_stds
        num_models, batch_size, _ = ensemble_model_means.shape
        models_to_use = np.random.choice(np.arange(num_models), size=batch_size)
        batch_inds = np.arange(batch_size)
        samples = ensemble_samples[models_to_use, batch_inds]
        rewards = float(samples[0, 0])
        next_obs = samples[0, 1:]
        return rewards, next_obs
